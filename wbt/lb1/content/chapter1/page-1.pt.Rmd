---
title: "Basics: Neues & Recap"  # Titel der Seite
...
In der praktischen Datenanalyse, in der wir Berechnungen nicht händisch durchführen (wie z. B. in der Vorlesung zu Statistik), nutzen wir die Rechnerleistung eines Computers, um Daten zu bearbeiten, einfache Berechnungen oder auch kompliziertere statische Modelle zu berechnen. Wir verwenden dabei Daten, die wir in Dateien gespeichert haben. Doch was sind eigentlich nochmal Dateien und wie lassen sich diese finden?

Daten werden in der Regel als **Dateien** an einem *lokalen* Ort auf dem Computer (Desktop oder Laptop) oder Tablet gespeichert. *Lokal* meint dabei, dass die Dateien physisch auf deinem Rechner abgelegt sind. Das Gegenteil Dateien, die zum Beispiel in einer Cloud (wie der Hessenbox) gespeichert sind und nur temporär bei Bearbeitung der Dateien lokal gespeichert werden. 
...
Auf dem Mac-Rechner sieht das wie folgt aus:
![Beispiel Pfad](../img/path.png)

Analog sieht es auf dem Windows-Rechner wie folgt aus:
![Beispiel Pfad Win](../img/path-win.png)
...
Einen wichtigen Unterschied sehen wir jetzt hier schon. Bei Windows-Rechnern mit deutscher Spracheinstellung wird ein Backslash `\` zur Unterscheidung von Ordnern genutzt, bei Mac-Rechnern ein einfacher Slash `/`. Das liegt an den Spracheinstellungen meines Windows-Rechner, der auf eine deutsche Systemsprache eingestellt ist. Wenn du mit einem Windows-Rechner arbeitest, ist es wichtig, dass diese Backslashes in der Benutzung in R immer zu normalen Slash umgestellt werden. Denn R arbeitet nur mit der englischen Spracheinstellung und in der werden Pfade immer mit Slashes `/` angegeben. 

Wir müssen also den oberen Pfad wie folgt ändern:
```
D:/g31442/Teaching/syllabus.pdf
```

Es ist wichtig, dass du diesen ersten Schritt verstanden hast. Am besten probierst du einmal selbst aus, den Dateifpad von einer beliebigen Datei in ein Word-Dokument zu kopieren. 

Falls du Fragen dazu hast, melde dich in der Präsenzzeit. Wir werden in den späteren Lernblöcken Pfade nutzen, um Daten in R zu bearbeiten und auch Ergebnisse aus R auf dem Computer zu speichern. 
